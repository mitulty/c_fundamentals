...
- A pointer is a varaible that contains the address of a variable. It is a group of cells that can hold an address. A pointer can be used to store the memory
  address of other variables, functions, or even other pointers. The use of pointers allows low-level memory access, dynamic memory allocation, and many other
  functionality in C.
...
- Pointer arithmetic is constant. All the pointer manipulations automatically take into account the size of the object pointed to. The valid pointer operations
...
- When an array name is passed to a function, what is passed is the location of the initial element. Within the called function, this argument is a local
  ...
  char *s are equivalent. When an array name is passed to a function, the function can at its convenience believe that it has been handed either an array or a
...
- The Null Pointers are those pointers that do not point to any memory location. They can be created by assigning a NULL value to the pointer. A pointer of any
  type can be assigned the NULL value.
                        data_type *pointer_name = NULL;
                                or
                        pointer_name = NULL
- The Void pointers in C are the pointers of type void. It means that they do not have any associated data type. They are also called generic pointers as they
  can point to any type and can be typecasted to any type.
                        void * pointer_name;
- In constant pointers, the memory address stored inside the pointer is constant and cannot be modified once it is defined. It will always point to the same
  memory address.
...
- The size of the pointers in C is equal for every pointer type. The size of the pointer does not depend on the type it is pointing to. It only depends on the
  operating system and CPU architecture. The size of pointers in C is 8 bytes for a 64-bit System and 4 bytes for a 32-bit System. The reason for the same size
  is that the pointers store the memory addresses, no matter what type they are. As the space required to store the addresses of the different memory locations
  is the same, the memory required by one pointer type will be equal to the memory required by other pointer types.
- Pointer arguments enable a function to access and change objects in the function that called it.
- The call by reference method of passing arguments to a function copies the address of an argument into the formal parameter. Inside the function, the address
...
int main(int argc, char const *argv[])
{
  ...
  *ip = *ip + 10; // incremenets x by 10
...

  {
    ...
    *arr = 100;
    for (int i = 0; i < 5; i++)
      printf("%d ", arr[i]);
    printf("\n");
    printf("arr:%p and p:%p\n", arr, p); // Array name holds the address of the first element of the array.
...

  {
    ...
    printf("strlen(s):%lu\n", strlen(s));
    char *t = "Hello world!!!";
    // char *uv = &s; // warning: initialization of ‘char *’ from incompatible pointer type ‘char **’
    char **uv = &s;
    char *uvwx = s;
    ...
    char u[] = {'H', 'i', '\0'};
    printf("strlen(u):%lu\n", strlen(u));
    // char *p = &u;     // warning: initialization of ‘char *’ from incompatible pointer type ‘char (*)[3]’
    // char **pp = &u; // warning: initialization of ‘char *’ from incompatible pointer type ‘char (*)[3]’
    char *p = u;
    char(*r)[3] = &u; // Here r is pointer to a character array of 3 elements.
    char *q = u;
    printf("u:%s and r:%s\n", u, *r); //  warning: format ‘%s’ expects argument of type ‘char *’, but argument 3 has type ‘char (*)[3]’
    char v[] = "Bye Bye World!!!";
    printf("s:%s, &s:%p, *s:%c\n", v, v, *v);
    // v = "Woo World!!!"; // error: assignment to expression with array type
    printf("Differenvce: sizeof(&v[0]):%lu, sizeof(v):%lu, sizeof(&v):%lu, sizeof(s):%lu\n", sizeof(&v[0]), sizeof(v), sizeof(&v), sizeof(s));
    printf("Differenvce: sizeof(&r[0]):%lu, sizeof(r):%lu, sizeof(&r):%lu, sizeof(u):%lu\n", sizeof(&r[0]), sizeof(r), sizeof(&r), sizeof(u));
    printf("&v:%p, &v+1:%p\n", &v, &v + 1);
    printf("r:%p, r+1:%p\n", r, r + 1);
    printf("*r[0]:%c and r:%s\n", *r[0], r);
    char *c = {'a', 'b'}; // warning: initialization of ‘char *’ from ‘int’ makes pointer from integer without a cast
    char carr[] = {'a', 'b'};
    char *c_ptr = carr;
  }
  {
    int x = 10;
    char ch = 'A';
    char *c = &x; // warning: initialization of ‘char *’ from incompatible pointer type ‘int *’
    char *d;
    int *p, *q;
    q = &x;
    p = q; // Assigning a pointer to another pointer of same type.
    d = p; // warning: initialization of ‘char *’ from incompatible pointer type ‘int *’
    int *e = (int *)&ch;
    int *f;
    f = (int *)c;
    char *g = (char *)&x;
  ...
  }
  {
    ...
    int(*r)[2];
    r = arr;
    int **s = arr;         // warning: initialization of ‘int **’ from incompatible pointer type ‘int (*)[2]’
    int **t = (int **)arr; // This is not a correct statement. t is a pointer to pointer which is a pointer to an int.
    printf("arr:%p, arr+1:%p, r:%p, r+1:%p, s:%p, s+1:%p\n", arr, arr + 1, r, r + 1, s, s + 1);
    printf("arr[1][1]:%d, r[1][1]:%d\n", arr[1][1], r[1][1]); // Accessing t[1][1] results in segementation fault.
    // printf("**t=%d\n", **t); // This results in segmentation fault.
    printf("t=%p, t+1=%p, *t=%d and arr=%p\n", t, t + 1, *t, arr);
  }
  {
    int arr[5] = {1, 2, 3, 4, 5};
    int *x = &arr; // warning: initialization of ‘int *’ from incompatible pointer type ‘int (*)[5]’
    int(*y)[5] = &arr;
    int **z = &arr; // warning: initialization of ‘int **’ from incompatible pointer type ‘int (*)[2]’
    printf("y:%p, y+1=%p\n", y, y + 1);
    printf("*y:%p, *y+1=%p\n", *y, *y + 1);
    printf("arr[3] with x:%d and arr[3] with y:%d\n", *(x + 3), *(*y + 3));
    // printf("arr[3] with z:%d", (**z + 3)); // Segmentation fault (core dumped)
  }
  {
    ...
    char **c_array = {
        ...
        "akhil"};
    int i = 0;
    printf("sizeof(names):%lu, sizeof(new_name):%lu and sizeof(c_array):%d\n", sizeof(names), sizeof(new_name), sizeof(c_array));
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      printf("new_name+i=%p and *(new_name+i):%s\t", (new_name + i), *(new_name + i));
      printf("c_array:%s\n", c_array);
    }
...
x:10, *p:10, *q:10
arr:0x7ffebf7db760, arr+1:0x7ffebf7db768, r:0x7ffebf7db760, r+1:0x7ffebf7db768, s:0x7ffebf7db760, s+1:0x7ffebf7db768
arr[1][1]:4, r[1][1]:4
arr[3] with x:4 and arr[3] with y:4
