...
- When the array is declared or allocated memory, the elements of the array contain some garbage value. So, the array is to be initialized to some meaningful
  value.There are multiple ways to initialize an array in C:
...
- Any array element can be accessed in C using the array subscript operator [] and the index value i of the element.
                                    array_name[index];
- The indexing in the array always starts with 0, i.e., the first element is at index 0 and the last element is at N – 1 where N is the number of elements in
  the array.
- Any array element can be updated at the given index i using the array subscript operator [] and assignment operator =.
                                    array_name[index] = new_value;
- An array can be traversed using the loops and the subscript operator using the correct index values.
- In C, the words, i.e., a sequence of characters are stored in the form of an array of characters terminated by a NULL character. These are called strings
  in C language.
...
- An array is always passed as pointers to a function in C. Whenever an arrray is passed to a function, it decays to the pointer and then passed as a pointer
  to the first element of an array.
- To return an array from a function, a pointer to the first element of that array is used.
